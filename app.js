...
function renderAttendance() {
  ...
  $("#staff-attendance-metrics").innerHTML = compactMetrics([
    ...
    { label: "Absent", value: String(absentStaff) },
    { label: "Total work time", value: totalDayMinutes ? `${Math.floor(totalDayMinutes / 60)}h ${String(totalDayMinutes % 60).padStart(2, "0")}m` : "—" }
  ]);
  ...
  const staffStatus = item => {
    const labels = {present:"Complete", absent:"Absent", half_day:"Half day", weekly_off:"Weekly off", weekly_off_present:"Worked weekly off", weekly_off_half_day:"Half day", holiday:"Holiday", leave:"Leave"};
    const value = item.attendanceStatus || (item.departureAt ? "present" : "incomplete");
    if (value === "absent") return '<span class="status status-absent">Absent</span>';
    if (["weekly_off", "holiday", "leave"].includes(value)) return `<span class="status status-neutral">${labels[value]}</span>`;
    if (["half_day", "weekly_off_half_day"].includes(value)) return `<span class="status status-review">${labels[value]}</span>`;
    if (value === "present" && (item.departureAt || Number(item.workDurationMinutes) > 0)) return '<span class="status status-active">Complete</span>';
    return `<span class="status status-review">${staffAttendanceDate === todayKey ? "Checked in" : "No checkout"}</span>`;
  };
  const staffDuration = item => {
    const reported = Number(item.workDurationMinutes);
    if (Number.isFinite(reported) && reported > 0) return `${Math.floor(reported / 60)}h ${String(reported % 60).padStart(2, "0")}m`;
    if (!item.departureAt || !item.arrivalAt) return "—";
    const minutes = Math.max(0, Math.round((asInstant(item.departureAt) - asInstant(item.arrivalAt)) / 60000));
    return `${Math.floor(minutes / 60)}h ${String(minutes % 60).padStart(2, "0")}m`;
  };
  const monthSummary = item => (state.staffAttendance?.monthlyTotals || []).find(summary => summary.month === staffAttendanceDate.slice(0, 7) && (summary.personKey === item.staffUserId || summary.personKey === `device:${item.deviceUserId}`));
  const monthlyWork = item => {
    const summary = monthSummary(item);
    if (!summary?.totalWorkMinutes) return "";
    const total = `${Math.floor(summary.totalWorkMinutes / 60)}h ${String(summary.totalWorkMinutes % 60).padStart(2, "0")}m month total`;
    const overtime = summary.overtimeMinutes ? ` · ${Math.floor(summary.overtimeMinutes / 60)}h ${String(summary.overtimeMinutes % 60).padStart(2, "0")}m OT` : "";
  ...
  const renderPeople = (prefix, records, singular, plural) => {
    ...
    $(`#${prefix}-attendance-table-body`).innerHTML = records.length ? records.map(item => `<tr><td>${staffIdentity(item)}</td><td><strong class="staff-time">${item.arrivalAt ? classTime(item.arrivalAt) : "—"}</strong></td><td><strong class="staff-time ${item.departureAt ? "" : "staff-time-muted"}">${item.departureAt ? classTime(item.departureAt) : "—"}</strong></td><td><span class="staff-duration"><strong>${staffDuration(item)}</strong>${monthlyWork(item) ? `<small>${esc(monthlyWork(item))}</small>` : ""}</span></td><td>${staffStatus(item)}</td></tr>`).join("") : `<tr><td colspan="5">${empty}</td></tr>`;
    $(`#${prefix}-attendance-mobile-list`).innerHTML = records.length ? records.map(item => `<article class="mobile-record-card staff-mobile-card"><div class="mobile-record-card-head">${staffIdentity(item)}${staffStatus(item)}</div><div class="mobile-record-meta"><div><span>Arrival</span><strong>${item.arrivalAt ? classTime(item.arrivalAt) : "—"}</strong></div><div><span>Departure</span><strong>${item.departureAt ? classTime(item.departureAt) : "—"}</strong></div><div><span>Work time</span><strong>${staffDuration(item)}</strong></div><div><span>Month total</span><strong>${monthSummary(item) ? `${Math.floor(monthSummary(item).totalWorkMinutes / 60)}h ${String(monthSummary(item).totalWorkMinutes % 60).padStart(2, "0")}m` : "—"}</strong></div></div></article>`).join("") : empty;
...
function renderCommunication() {
  ...
  $("#notice-list").innerHTML = state.notices.length ? state.notices.map(item => `<article class="surface notice-card"><div class="notice-card-head"><span class="icon-tile">${icon("message")}</span><div class="cell-actions">${status(item.deliveryStatus || item.status)}${ownerEditButton("notice", item.id)}</div></div><h3>${esc(item.title)}</h3><p>${esc(item.body)}</p><footer><span>${esc(item.batch ? `${item.batch} · ${item.program || "All programs"}${item.subject ? ` · ${item.subject}` : ""}` : item.audience)}</span><span>${esc(item.channel.replaceAll("_", " "))}</span><time>${formatDateTime(item.publishedAt || item.createdAt)}</time></footer></article>`).join("") : emptyState("message", "No notices");
}
...
function openStudentCreateForm() {
  ...
  openDrawer("Add student", `<form class="auth-form" id="student-create-form">
    ...
    <fieldset class="choice-fieldset"><legend>Subjects</legend><div class="choice-grid">${STUDENT_SUBJECTS.map(subject => `<label class="check-field"><input name="subjects" type="checkbox" value="${subject}"><span>${subject}</span></label>`).join("")}</div><small>Select every subject this student will attend.</small></fieldset>
...
function openLeadConversionForm(lead) {
  ...
  openDrawer(`Convert admission · ${lead.student}`, `<form class="auth-form" id="lead-conversion-form" data-lead-id="${esc(lead.id)}">
    ...
    <fieldset class="choice-fieldset"><legend>Subjects</legend><div class="choice-grid">${STUDENT_SUBJECTS.map(subject => `<label class="check-field"><input name="subjects" type="checkbox" value="${subject}"><span>${subject}</span></label>`).join("")}</div></fieldset>
    <label class="field"><span>Agreed course fee</span><input name="agreedAmount" type="number" min="0" step="1" inputmode="numeric" required></label>
...
function openNoticeForm() {
  openDrawer("New notice", `<form class="auth-form" id="notice-form"><label class="field"><span>Title</span><input name="title" minlength="2" maxlength="255" required></label><label class="field"><span>Message</span><textarea name="body" rows="5" minlength="2" maxlength="5000" required></textarea></label><label class="field"><span>Audience</span><select name="audience"><option value="all">Everyone</option><option value="parents">Parents</option><option value="students">Students</option><option value="faculty">Faculty</option><option value="batch">Batch</option><option value="subject">Batch and subject</option></select></label><label class="field"><span>Batch <small>(for batch or subject)</small></span><select name="batchId"><option value="">Not selected</option>${options(state.timetable.batches || [], item => `${item.name} · ${item.program}`)}</select></label><label class="field"><span>Subject <small>(for subject audience)</small></span><select name="subjectId"><option value="">Not selected</option>${options(state.timetable.subjects || [], item => item.name)}</select></label><label class="field"><span>Channel</span><select name="channel"><option value="in_app">In app</option><option value="email" disabled>Email · provider not connected</option><option value="sms" disabled>SMS · provider not connected</option><option value="whatsapp" disabled>WhatsApp · provider not connected</option></select></label><div class="inline-notice">${icon("info")}<span>In-app announcements also create a private device alert for users who enable notifications.</span></div><label class="field"><span>Status</span><select name="status"><option value="published">Publish now</option><option value="draft">Save draft</option></select></label>${formError("notice-form-error")}<button class="button button-primary button-large" type="submit">${icon("message")}Save notice</button></form>`);
  $("#notice-form").addEventListener("submit", submitNotice);
}

async function submitNotice(event) {
  event.preventDefault(); const form = new FormData(event.currentTarget), button = $("button[type=submit]", event.currentTarget); button.disabled = true;
  const payload = Object.fromEntries(["title", "body", "audience", "channel", "status"].map(key => [key, String(form.get(key)).trim()])); payload.batchId = String(form.get("batchId") || "") || null; payload.subjectId = String(form.get("subjectId") || "") || null;
...
async function openOwnerEdit(kind, id) {
  ...
  if (kind === "student") {
    ...
    fields = `<div class="form-pair"><label class="field"><span>Admission number</span><input name="admissionNumber" value="${esc(item.admissionNumber)}" required></label><label class="field"><span>Student name</span><input name="fullName" value="${esc(item.fullName)}" required></label></div>
      ...
      <fieldset class="choice-fieldset"><legend>Subjects</legend><div class="choice-grid">${STUDENT_SUBJECTS.map(subject => `<label class="check-field"><input name="subjects" type="checkbox" value="${subject}"${checked(item.academicProfile?.subjects?.includes(subject))}><span>${subject}</span></label>`).join("")}</div></fieldset>`;
  } else if (kind === "lead") {
    title = "Edit enquiry";
