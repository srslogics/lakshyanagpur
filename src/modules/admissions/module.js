(function () {
  const admissionsModuleData = {
    ...
    leads: [
      ...
      },
      {
        ...
        summary: "Student is already comparing repeaters programs. Visit quality and mentorship clarity will decide the conversion.",
        tasks: [
          "Reserve front-row visit seat",
          "Send NEET repeaters brochure to parent",
          "Schedule post-visit feedback call"
        ],
        timeline: [
          "School seminar lead imported 2 days ago",
          "Counselling call completed yesterday",
          "Biology visit seat confirmed for tomorrow"
        ]
      },
      {
        id: "lead-vedant-zade",
        ...
        tasks: [
          ...
          "Hold Sunday seat till payment"
        ],
        timeline: [
          "Instagram lead captured 3 days ago",
          "Visit completed with positive feedback",
  ...
  const state = {
    ...
    stage: "all",
    counsellor: "all",
    selectedLeadId: admissionsModuleData.leads[0] ? admissionsModuleData.leads[0].id : null
  };
  ...
  function buildDynamicStats() {
    ...
    const visits = state.leads.filter(lead => lead.stage === "Demo Booked").length;
    return {
      kpis: [
        { label: "Hot Leads", value: String(hot), note: "Need same-day touch" },
        { label: "Visits Booked", value: String(visits), note: "Ready for parent visit" },
        { label: "Open Pipeline", value: String(total - won), note: "Still in active conversion" }
      ],
      stats: [
      ...
      return `
        <div class="stage-card">
          ...
          <div class="stage-track">
            <div class="stage-fill" style="width:${Math.round((count / totalActive) * 100)}%"></div>
          </div>
        </div>
      `;
    }).join("");
  ...
  function renderSourceList(target) {
    ...
    target.innerHTML = [...sourceMap.values()].sort((a, b) => b.leads - a.leads).map(item => `
      <div class="source-item">
        <div>
          <strong>${item.source}</strong>
          <span>${item.leads} leads</span>
        </div>
        <strong>${Math.round((item.won / Math.max(1, item.leads)) * 100)}%</strong>
      </div>
    `).join("");
  }
  ...
  function buildCounsellorLoad() {
    ...
    state.leads.forEach(lead => {
      const current = counters.get(lead.counsellor) || { total: 0, hot: 0, visits: 0, won: 0 };
      current.total += 1;
      if (lead.stage === "Hot") current.hot += 1;
      if (lead.stage === "Demo Booked") current.visits += 1;
      if (lead.stage === "Won") current.won += 1;
      counters.set(lead.counsellor, current);
    });
    return [...counters.entries()].map(([title, counts]) => ({
      title,
      note: `${counts.total} active leads | ${counts.hot} hot | ${counts.visits} visits | ${counts.won} won`
    }));
  }

  function buildQueue() {
    return state.leads.filter(lead => lead.stage !== "Won").slice(0, 4).map(lead => ({
  ...
  function getFilteredLeads() {
    return state.leads.filter(lead => {
      const matchesSearch = !state.search || [lead.student, lead.parent, lead.program, lead.source, lead.counsellor]
  ...
  function renderLeadsTable(target, leads) {
    if (!target) return;
    if (!leads.length) {
      target.innerHTML = `<tr><td colspan="7"><div class="table-empty"><strong>No leads match the current filters.</strong><span>Try another counsellor, stage, or search term.</span></div></td></tr>`;
      return;
    }
    target.innerHTML = leads.map(lead => `
      <tr class="lead-row${lead.id === state.selectedLeadId ? " selected" : ""}" data-lead-id="${lead.id}">
        <td><strong>${lead.student}</strong></td>
        <td>${lead.program}</td>
        <td>${lead.parent}</td>
        <td>${lead.source}</td>
  ...
  function renderSelectedLead(target, lead) {
    if (!target) return;
    if (!lead) {
      target.innerHTML = `<div class="feature-empty"><strong>No active lead selected</strong><span>Choose a lead from the register to inspect the full profile.</span></div>`;
      return;
    }
    target.innerHTML = `
      ...
      <div class="lead-meta-grid">
        <div class="lead-meta-card"><small>Parent</small><strong>${lead.parent}</strong></div>
        <div class="lead-meta-card"><small>Counsellor</small><strong>${lead.counsellor}</strong></div>
        <div class="lead-meta-card"><small>Commercial</small><strong>${lead.budget}</strong></div>
        <div class="lead-meta-card"><small>Last touch</small><strong>${lead.lastTouch}</strong></div>
      </div>
      <p>${lead.summary}</p>
      <div class="lead-detail-block">
        <h4>Immediate actions</h4>
        <ul class="feature-list">${lead.tasks.map(item => `<li>${item}</li>`).join("")}</ul>
      </div>
      <div class="lead-detail-block">
        <h4>Stage actions</h4>
        <div class="stage-actions">
          ${STAGE_ORDER.map(stage => `<button class="pill${lead.stage === stage ? " pill-primary" : ""}" type="button" data-stage-action="${stage}">${stage}</button>`).join("")}
        </div>
      </div>
      <div class="lead-detail-block">
      ...
      <div class="lead-detail-block">
        ...
        <ul class="lead-mini-timeline">${lead.timeline.map(item => `<li>${item}</li>`).join("")}</ul>
      </div>
    `;
  }
  ...
  function populateFilterOptions(refs) {
    ...
    refs.stage.innerHTML = `<option value="all">All stages</option>${stages.map(stage => `<option value="${stage}">${stage}</option>`).join("")}`;
    refs.counsellor.innerHTML = `<option value="all">All counsellors</option>${counsellors.map(name => `<option value="${name}">${name}</option>`).join("")}`;
  }

  function populateCounsellorInput(refs) {
    if (!refs.createCounsellor) return;
    const counsellors = [...new Set(state.leads.map(lead => lead.counsellor))];
    refs.createCounsellor.innerHTML = `<option value="">Assign counsellor</option>${counsellors.map(name => `<option value="${name}">${name}</option>`).join("")}`;
  }
  ...
  function bindWorkspace(refs) {
    refs.search && refs.search.addEventListener("input", event => {
      state.search = event.target.value.trim().toLowerCase();
      renderWorkspace(refs);
    });
    refs.stage && refs.stage.addEventListener("change", event => {
      state.stage = event.target.value;
      renderWorkspace(refs);
    });
    refs.counsellor && refs.counsellor.addEventListener("change", event => {
      state.counsellor = event.target.value;
      renderWorkspace(refs);
    });
    refs.reset && refs.reset.addEventListener("click", () => {
      state.search = "";
      state.stage = "all";
    ...
    refs.rows && refs.rows.addEventListener("click", event => {
      const row = event.target.closest("[data-lead-id]");
      if (!row) return;
      state.selectedLeadId = row.dataset.leadId;
      renderWorkspace(refs);
    });
    refs.createForm && refs.createForm.addEventListener("submit", event => {
    ...
    refs.selected && refs.selected.addEventListener("click", event => {
    ...
    refs.selected && refs.selected.addEventListener("submit", event => {
      ...
      if (note) selectedLead.timeline.unshift(note);
      if (nextAction) {
  ...
  window.LakshyaAdmissionsModule = { initAdmissionsModule };
})();
