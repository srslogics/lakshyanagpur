...
def seed_development_data(db):
    ...
    if not db.query(User).first():
        db.add_all([
            User(mobile="9000000001", email="owner@lakshyanagpur.in", full_name="Lakshya Director", role="owner", password_hash=hash_password("ChangeMe123!")),
            User(mobile="9000000002", email="counsellor@lakshyanagpur.in", full_name="Priya Kulkarni", role="counsellor", password_hash=hash_password("ChangeMe123!")),
        ]); db.flush()
