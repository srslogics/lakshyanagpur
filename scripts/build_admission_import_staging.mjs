...
const amountFrom = part => {
  ...
  const plain = part.match(/(\d+(?:\.\d+)?)\s*(?=upi|cash)/i);
...
const paymentMode = part => /cash/i.test(part) ? "Cash" : /upi/i.test(part) ? "UPI" : "Unknown";
const normalizeCourse = value => ({"JEE":"JEE", "NEET":"NEET", "MHT-CET":"MHT-CET", "BOARDS":"Boards"}[asText(value).toUpperCase()] || asText(value));
...
for (let i = 2; i < raw.length; i++) {
  ...
  if (contactList.length > 1) issues.push("Confirm which contact is student vs guardian");
  ...
  records.push({
    ...
    readiness: section === "cancelled" ? "DO NOT IMPORT" : blocker ? "BLOCKED" : issues.length ? "REVIEW" : "READY",
    parsedTotal,
  });
}
...
for (const record of active) for (const phone of [record.primaryMobile, record.secondaryMobile].filter(Boolean)) {
...
for (const record of active) {
  ...
  if (duplicates.length) { record.issues.push(`Duplicate contact across active rows: ${duplicates.join(", ")}`); record.readiness = "BLOCKED"; }
}
...
for (const record of records) {
  paymentParts(record.remarks).forEach((part, index) => {
    ...
    payments.push({ legacyId: record.legacyId, sourceRow: record.sourceRow, line: index + 1, student: record.student, status: record.status, date, amount: amount || 0, mode: paymentMode(part), kind: incentive ? "Incentive / concession review" : "Payment", raw: part, readiness: record.status === "cancelled" ? "DO NOT IMPORT" : incentive || !date || paymentMode(part) === "Unknown" ? "REVIEW" : "READY" });
  });
}
...
mapping.getRange("A4:F13").values = [
  ["Sr. No","Migration audit","legacy_id","Prefix with LEGACY-ADM-2026-A/C","Always","Not used as the ERP primary key"],
  ...
  ["Contact number","Student + Guardian","mobile fields","Extract one or two 10-digit numbers","Block if blank","Relationship is unknown; verify before guardian linking"],
  ["Course","Program/Enrollment","program_code","Map JEE, NEET, MHT-CET, Boards","Required","Batch is absent and remains unassigned"],
  ...
  ["Remarks","Payment staging","raw_payment_note","Split +/line-separated entries","Manual review","Modes/dates/incentives are mixed in free text"],
];
...
mapping.getRange("A16:F20").values = [
  ["1","Student","Create central student record","Only READY rows","Never create cancelled records",null],
  ["2","Guardian","Create unverified guardian link","After contact relationship review","Parent/guardian name is not present in source",null],
  ["3","Enrollment","Create active enrollment with batch unassigned","After program mapping","Batch must be assigned in ERP",null],
...
summary.getRange("A4:A12").values = [["Active admissions"],["Cancelled admissions"],["Active fee agreements"],["Active registration total"],["Ready rows"],["Review rows"],["Blocked rows"],["Missing contact numbers"],["Multi-number contacts"]];
summary.getRange("B4:B12").formulas = [["=COUNTIF('Students Import'!$B$4:$B$73,\"Active\")"],["=COUNTIF('Students Import'!$B$4:$B$73,\"Cancelled\")"],["=SUMIF('Students Import'!$B$4:$B$73,\"Active\",'Students Import'!$K$4:$K$73)"],["=SUMIF('Students Import'!$B$4:$B$73,\"Active\",'Students Import'!$L$4:$L$73)"],["=COUNTIF('Students Import'!$N$4:$N$73,\"READY\")"],["=COUNTIF('Students Import'!$N$4:$N$73,\"REVIEW\")"],["=COUNTIF('Students Import'!$N$4:$N$73,\"BLOCKED\")"],["=COUNTBLANK('Students Import'!$G$4:$G$67)"],["=SUM('Students Import'!$R$4:$R$67)"]];
...
summary.getRange("D4:H10").merge(true); summary.getRange("D4:D10").values = [["Cancelled admissions remain audit-only and must not create students or payments."],["Rows without a contact number are blocked because duplicate detection and family linking require one."],["Two-number cells need a student-versus-guardian decision; the workbook does not identify relationships."],["Admission Lead is attribution data, not a reliable enquiry source. Raw values are preserved for mapping."],["Batch, guardian name, guardian relationship, receipt number, and verified payment reference are absent."],["Incentives must become approved adjustments, not payment receipts."],["Only rows marked READY should enter the first automated import run."]];
...
paymentSheet.getRangeByIndexes(3,0,payments.length,paymentHeaders.length).values = payments.map(p => [p.legacyId,p.sourceRow,p.line,p.student,p.status === "active" ? "Active" : "Cancelled",p.date,p.amount,p.mode,p.kind,p.raw,p.readiness,"",""]);
paymentSheet.getRange(`F4:F${3+payments.length}`).format.numberFormat = "yyyy-mm-dd"; paymentSheet.getRange(`G4:G${3+payments.length}`).format.numberFormat = moneyFormat;
...
const exceptionRows = records.filter(r => r.issues.length || r.status === "cancelled");
...
const exceptionLines = exceptionRows.flatMap(r => (r.issues.length ? r.issues : ["Cancelled source record — retain for audit only"]).map(issue => [r.legacyId,r.sourceRow,r.student,r.readiness,issue,"No",""]));
...
await fs.writeFile(`${previewDir}/checks.json`, JSON.stringify({ counts: { active: active.length, cancelled: cancelled.length, payments: payments.length, exceptions: exceptionLines.length, feeTotal: active.reduce((a,r)=>a+r.feeTotal,0), registrationTotal: active.reduce((a,r)=>a+r.registrationTotal,0), ready: active.filter(r=>r.readiness==="READY").length, review: active.filter(r=>r.readiness==="REVIEW").length, blocked: active.filter(r=>r.readiness==="BLOCKED").length }, errors, checks }, null, 2));
console.log(JSON.stringify({ outputPath, manifestPath, sourceHash, active: active.length, cancelled: cancelled.length, payments: payments.length, stagedPaymentTotal: manifest.expected.staged_payment_total, exceptions: exceptionLines.length, feeTotal: active.reduce((a,r)=>a+r.feeTotal,0), registrationTotal: active.reduce((a,r)=>a+r.registrationTotal,0), ready: active.filter(r=>r.readiness==="READY").length, review: active.filter(r=>r.readiness==="REVIEW").length, blocked: active.filter(r=>r.readiness==="BLOCKED").length, errors }, null, 2));
